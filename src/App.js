...
const METRICS = {
  batting: [
    { id: 'hr', label: '本塁打', color: '#f87171', icon: <Trophy size={14} />, desc: '打球をスタンドに叩き込む数。圧倒的なパワーの証拠になります。', ranks: { team: 1, league: 6, mlb: 12 }, mlbAvg: 15, thresholds: [15, 30, 40] },
    { id: 'rbi', label: '打点', color: '#fb923c', icon: <Target size={14} />, desc: '自分のバットでランナーを返し、チームに得点を入れた数。', ranks: { team: 2, league: 8, mlb: 15 }, mlbAvg: 55, thresholds: [55, 85, 100] },
    { id: 'sb', label: '盗塁', color: '#fbbf24', icon: <Zap size={14} />, desc: 'ピッチャーの隙を突いて次の塁を奪う数。足の速さと賢さの証。', ranks: { team: 2, league: 12, mlb: 25 }, mlbAvg: 8, thresholds: [8, 20, 35] },
    ...
    { id: 'bb', label: '四死球', color: '#a78bfa', icon: <Footprints size={14} />, desc: 'ボールを見極めて歩いた数。相手から恐れられている証拠です。', ranks: { team: 1, league: 4, mlb: 8 }, mlbAvg: 40, thresholds: [40, 70, 90] },
    { id: 'b_war', label: '野手WAR', color: '#f472b6', icon: <Star size={14} />, desc: 'どれだけ勝利に貢献したかを総合的に表す究極の指標。', ranks: { team: 1, league: 2, mlb: 3 }, mlbAvg: 2.0, thresholds: [2.0, 4.5, 7.0] },
  ],
  pitching: [
    ...
    { id: 'ip', label: '投球回', color: '#8b5cf6', icon: <Clock size={14} />, desc: 'マウンドでアウトを取った回数。チームの大黒柱の証。', ranks: { team: 2, league: 15, mlb: 30 }, mlbAvg: 140, thresholds: [140, 180, 200] },
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#050914] text-slate-200 p-4 flex flex-col gap-6">
      <header className="max-w-6xl w-full mx-auto flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
        ...
        <div className="bg-slate-900/80 border border-slate-800 p-3 rounded-lg text-xs font-mono">
          <div className="flex gap-4">
            <span>消化: <span className="text-white">{ESTIMATED_GAMES_PLAYED} 試合</span></span>
            <span>残り: <span className="text-blue-400">{REMAINING_GAMES} 試合</span></span>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-6xl w-full mx-auto flex flex-col gap-6">
        <div className="flex flex-col gap-3">
          <div className="flex overflow-x-auto gap-2 pb-2 [&::-webkit-scrollbar]:h-1.5 [&::-webkit-scrollbar-thumb]:bg-slate-700">
            <div className="px-3 py-2 bg-red-950/40 text-red-400 text-[10px] font-bold rounded-lg shrink-0">BATTING</div>
            {METRICS.batting.map(m => (
              <button key={m.id} onClick={() => setActiveMetricId(m.id)} className={`px-4 py-2 rounded-xl border shrink-0 ${activeMetricId === m.id ? 'bg-blue-600/20 border-blue-500' : 'bg-slate-900 border-slate-800'}`}>
                <div className="text-[10px] text-slate-400">{m.label}</div>
                <div className="text-lg font-black">{currentStat[m.id]}</div>
              </button>
            ))}
          </div>
          <div className="flex overflow-x-auto gap-2 pb-2 [&::-webkit-scrollbar]:h-1.5 [&::-webkit-scrollbar-thumb]:bg-slate-700">
            <div className="px-3 py-2 bg-emerald-950/40 text-emerald-400 text-[10px] font-bold rounded-lg shrink-0">PITCHING</div>
        ...
        <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-5 flex flex-col gap-6">
          <div className="flex justify-between items-center border-b border-slate-800 pb-4">
            <h2 className="text-xl font-black">{activeMetricObj.label} 詳細分析</h2>
            <div className="text-4xl font-black italic text-blue-400">{getProjectedValue(activeMetricId)}<span className="text-xs not-italic text-slate-500 ml-2 uppercase">Proj.</span></div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-[300px]">
            <div className="bg-black/20 p-4 rounded-xl relative">
              <div className="flex justify-between mb-2">
                <span className="text-[10px] font-bold text-slate-500 uppercase">Daily Tracker</span>
                <button onClick={() => setShowTodayCompare(!showTodayCompare)} className="text-[10px] bg-slate-800 px-2 py-1 rounded">4/17比較</button>
              </div>
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={paceData}>
                  <CartesianGrid stroke="#1e293b" vertical={false} />
                  <XAxis dataKey="label" fontSize={9} interval={30} />
                  <YAxis fontSize={9} hide={true} reversed={activeMetricId === 'era'} />
                  <Line type="monotone" dataKey="2026" stroke="#3b82f6" strokeWidth={3} dot={false} />
                  <Line type="monotone" dataKey="2024" stroke="#f43f5e" strokeWidth={1} dot={false} opacity={0.5} />
                </LineChart>
              </ResponsiveContainer>
              {showTodayCompare && (
                <div className="absolute top-10 right-4 bg-slate-900 p-3 rounded border border-slate-700 z-50 text-[10px]">
                  <div>2026: {todayData['2026']}</div>
                  <div>2024: {todayData['2024']}</div>
                </div>
              )}
            </div>

            <div className="bg-black/20 p-4 rounded-xl">
              <span className="text-[10px] font-bold text-slate-500 uppercase">Annual Comparison</span>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={annualData} margin={{ top: 20, left: -20 }}>
                  <CartesianGrid stroke="#1e293b" vertical={false} />
                  <XAxis dataKey="year" fontSize={9} />
                  <YAxis fontSize={9} domain={[0, calculatedMax]} />
                  <ReferenceLine y={activeMetricObj.mlbAvg} stroke="#475569" strokeDasharray="3 3" />
                  <Bar dataKey={activeMetricId} stackId="a">
      ...
      <footer className="text-center text-[10px] text-slate-600 font-mono mt-auto">
        SHO-TIME TRACKER • 2026 SEASON ANALYTICS
      </footer>
    </div>
  );
}
